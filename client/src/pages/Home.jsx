    ...
    <div className="bg-background text-text">
      ...
      >
        ...
        <div className="relative z-10">
          ...
          <button className="bg-accent text-text px-8 py-3 rounded-full font-semibold shadow hover:scale-105 transform transition">
            {t.cta}
          </button>
        </div>
      </section>
  ...
  <div className="flex flex-col md:flex-row justify-center items-center gap-8 px-4">
    {t.services.map((service, index) => (
      <div key={index} className="bg-white p-8 rounded-3xl shadow-xl w-full max-w-sm text-center">
        <h3 className="text-2xl font-semibold mb-3 text-secondary">{service}</h3>
        <p className="text-gray-600">En kort beskrivning av {service.toLowerCase()} här.</p>
      </div>
    ))}
  </div>
</section>
