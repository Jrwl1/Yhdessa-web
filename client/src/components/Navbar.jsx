...
const FlagFI = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 480" className="h-6 w-6">
...
const FlagGB = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 480" className="h-6 w-6">
    ...
    <path fill="#fff" d="M0 0l640 480m0-480L0 480" stroke="#fff" strokeWidth="72" />
    <path fill="#c8102e" d="M0 0l640 480m0-480L0 480" stroke="#c8102e" strokeWidth="38" />
    <path fill="#fff" d="M270 0h100v480H270zM0 190h640v100H0z" />
    <path fill="#c8102e" d="M295 0h50v480h-50zM0 215h640v50H0z" />
  </svg>
);
...
export default function Navbar() {
  ...
  const navClass = ({ isActive }) =>
    `rounded-full px-4 py-2 text-sm transition md:text-base ${
      ...
      <nav className="mx-auto w-full max-w-6xl px-4 md:px-6">
        <div className="flex flex-col items-center gap-2 md:relative md:flex-row md:justify-center">
          ...
          <div className="flex items-center gap-2 md:absolute md:right-0 md:top-3">
            <button onClick={() => setLanguage("fi")} aria-label="Finnish" className="rounded-full bg-white/70 p-1.5 transition hover:bg-white">
              <FlagFI />
            </button>
            <button onClick={() => setLanguage("sv")} aria-label="Swedish" className="rounded-full bg-white/70 p-1.5 transition hover:bg-white">
              <FlagSE />
            </button>
            <button onClick={() => setLanguage("en")} aria-label="English" className="rounded-full bg-white/70 p-1.5 transition hover:bg-white">
              <FlagGB />
            </button>
          </div>
        </div>
